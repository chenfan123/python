...
不确定调用的时候会传递多少个参数，不传参也可以，用*args表示接收不定长位置参数，**kwargs表示接收不定长关键字参数
```python
...
#### lambda 匿名函数
- def关键字可以定义带有名称的函数，lambda关键字可以定义匿名函数（无名称）
...
#### read()方法
文件对象.read(num)
文件对象.read(n)：读取文件内容，返回字符串，n表示读取的字节数,如果没有传num就表示读取文件中所有。
...
### 文件写入
#### 直接调用write，write方法必须传递字符串
但是并未真实写入，是积攒在内存中，称之为缓冲区。
调用flush的时候，内容会真正写入文件。
为了避免频繁的操作硬盘，导致效率下降

**因为硬盘是低速设备，程序是cpu执行，速度极快。所以未来避免低速设备拉慢程序的执行，在操作的时候会在内存中开辟缓冲区，作用是将要写入硬盘的数据先积攒在缓冲区中，当缓冲区满、或者调用flush、或调用close的时候，数据才会从缓冲区真正写入硬盘。**

> 只有文本文件可以r w a，非文本文件需要用b，以二进制模式处理
